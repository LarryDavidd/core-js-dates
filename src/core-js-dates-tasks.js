...
function getNextFriday(date) {
  ...
  return new Date(
    ...
    newDate.getMonth(),
    newDate.getDate(),
    newDate.getHours() + newDate.getTimezoneOffset() / -60
  );
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (new Date(dateEnd) - new Date(dateStart)) / (1000 * 3600 * 24) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const newDate = new Date(date);
  let res = false;
  if (newDate >= start && newDate <= end) res = true;
...
function formatDate(date) {
  ...
  return new Intl.DateTimeFormat('en-US', times).format(new Date(date));
}
...
function getNextFridayThe13th(date) {
  let newDate = new Date(date.getFullYear(), date.getMonth(), 13);
  while (newDate.getDay() !== 5) {
    newDate = new Date(newDate.getFullYear(), newDate.getMonth() + 1, 13);
  }
  return newDate;
}
